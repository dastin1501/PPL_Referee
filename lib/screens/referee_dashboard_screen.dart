...
class _RefereeDashboardScreenState extends State<RefereeDashboardScreen> {
  ...
  bool _isRallyScoringForSelectedGame() {
    return context.read<AppState>().selectedGame?.isRallyScoring ?? false;
  }

  String _scoringFormatLabel(TournamentMatch match) {
    return match.isRallyScoring
        ? 'Scoring Format: Rally'
        : 'Scoring Format: Side-Out';
  }
  ...
  void _checkCoinToss() {
    ...
    if (g != null) {
      // Prefer per-game scheduled scores for the selected game; default 0-0
      int assignedFromList = context.read<AppState>().selectedGameNumber;
      int cg = _currentGame;
  ...
  void _toggleReceiverSide(TournamentMatch g) {
    ...
    if (leftTeam.contains(_servingPlayer)) {
      // Anchor right team: base stays top, second stays bottom
      ...
      // Anchor left team: base stays bottom, second stays top
  ...
  void _toggleLeftReceiver(TournamentMatch g) {
    ...
    if (!_gameStarted) {
      if (_leftBase.isEmpty || _leftSecond.isEmpty) {
        ...
        _leftBase = _leftBottomOverride;
        _leftSecond = _leftTopOverride;
      } else {
        final tmpBase = _leftBase;
        _leftBase = _leftSecond;
        _leftSecond = tmpBase;
        _leftTopOverride = _leftSecond;
        _leftBottomOverride = _leftBase;
      }
  ...
  void _toggleRightReceiver(TournamentMatch g) {
    ...
    if (!_gameStarted) {
      if (_rightBase.isEmpty || _rightSecond.isEmpty) {
        ...
        final tmpBase = _rightBase;
        _rightBase = _rightSecond;
        _rightSecond = tmpBase;
        _rightTopOverride = _rightBase;
        _rightBottomOverride = _rightSecond;
      }
      setState(() {});
